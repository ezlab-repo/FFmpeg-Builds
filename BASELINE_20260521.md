...
본 문서는 patent-clean 빌드를 만들기 *직전* BtbN 본가 LGPL shared 빌드의 *원본 상태 스냅샷*. 향후 BtbN 본가 sync 후 새 빌드와 비교하여 신규 활성 특허 코덱·라이브러리 감지에 사용.

## 기준 정보

| 항목 | 값 |
|---|---|
| Fork 시점 BtbN 본가 master commit | `7b5dc8c88b43` (2026-05-18) |
| 우리 fork 위치 | github.com/ezlab-repo/FFmpeg-Builds |
| BtbN 공식 빌드 다운로드 시점 | 2026-05-21 |
| 사용한 BtbN 빌드 파일명 | `ffmpeg-master-latest-win64-lgpl-shared` |
| FFmpeg 버전 | `N-124557-g9e71ea2d60-20260520` |
| 메이저 라이브러리 버전 | avcodec-62, avformat-62, avutil-60, avfilter-11, swresample-6, swscale-9, avdevice-62 |
...
**결과: 없음 (안전)**. `${{ secrets.X }}` 형태의 *사용자 정의 secret* 참조 없음. `${{ github.token }}`만 사용 — 이는 모든 fork에 자동 발급되는 토큰이라 우리 fork에서도 정상 동작.

### Docker 이미지 의존

**결과: 안전 (자체 빌드)**. 워크플로우는 외부 사전 빌드 이미지 pull에 의존하지 않음. 빌드 시점에 자체적으로 Docker 이미지를 만들고 GHCR에 push.

### Matrix 구조 (BtbN 본가 원본)
...
→ 4 × 12 = 48 조합 (+ 일부 win64-only 추가). 우리 fork에서는 `target: [win64]`, `variant: [lgpl-shared]`로 좁힘.

### 결론

R-1 (workflow secrets 의존): 안전. R-2 (Docker 이미지 출처·접근권): 안전. → Phase B 변경 후 push로 빌드 트리거 시 기술적 장애 없음.
...
Encoders:
 ...
 V....D av1_mf               AV1 via MediaFoundation (codec av1)
...
Decoders:
 ...
 VFS..D apv                  Advanced Professional Video
 ...
 V..... vp9_amf              VP9 AMD AMF video decoder (codec vp9)
 ...
 S..... dvdsub               DVD subtitles (codec dvd_subtitle)
 S..... cc_dec               Closed Captions (EIA-608 / CEA-708) (codec eia_608)
