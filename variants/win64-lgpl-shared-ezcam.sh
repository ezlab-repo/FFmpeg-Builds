...
# ezCapture 빌드(win64-lgpl-shared.sh)와 동일한 patent-clean 베이스 +
# H.264 인코더 중 h264_mf 한 종만 추가 허용.
#
# h264_mf 허용 근거:
#   - Windows Media Foundation API 래퍼 → Windows EULA의 H.264 라이선스에 위임
#   - ezCam은 Windows 전용 데스크톱 녹화 앱 → 사용 PC가 곧 라이선스 보유 주체
#   - libopenh264·h264_nvenc·h264_amf·h264_qsv·h264_d3d12va·h264_vaapi·h264_vulkan
#     은 모두 사용하지 않으므로 disable 유지 (특허 노출 축소)
#
# 자세한 내용은 fork 레포 루트의 DISABLED_CODECS.md "ezCam 빌드 차이" 섹션 참고

# ezCam은 MP4 출력 사용 — 명시적 enable로 의존성 누락 회피
...
# H.264 / AVC — h264_mf만 활성, 나머지는 비활성 (Via Licensing AVC)
# libopenh264는 scripts.d/50-openh264.sh에서 별도 disable
FF_CONFIGURE+=" --disable-decoder=h264,h264_qsv,h264_cuvid,h264_amf"
FF_CONFIGURE+=" --disable-encoder=h264_amf,h264_qsv,h264_nvenc,h264_d3d12va,h264_vaapi,h264_vulkan"
FF_CONFIGURE+=" --enable-encoder=h264_mf"  # ezCam이 사용하는 OS 위임 인코더 (Windows MF)
FF_CONFIGURE+=" --disable-parser=h264"
FF_CONFIGURE+=" --disable-bsf=h264_metadata,h264_redundant_pps"  # h264_mp4toannexb는 보수적으로 제외

# VVC (H.266) 제거 — 최신 표준, 활성 특허
FF_CONFIGURE+=" --disable-decoder=vvc,vvc_qsv"
FF_CONFIGURE+=" --disable-parser=vvc"
FF_CONFIGURE+=" --disable-bsf=vvc_metadata"  # vvc_mp4toannexb는 보수적으로 제외
...
# Apple ProRes 제거 (Apple 비인증 구현, ezCam 사용 안 함)
